...
마우스로 worktree를 선택하고, 서버 실행·종료와 TCP 포트를 확인할 수 있습니다. 서버 로그는 CLI에서 조회합니다.
...
알림은 우측 상단에 하나씩 겹쳐 표시되어 패널이나 하단 버튼 위치를 바꾸지 않습니다.
완료 알림은 3초 뒤 사라지고, 진행 알림은 작업이 끝날 때까지 유지됩니다.
오류·경고는 `Esc`로 닫습니다. 다이얼로그가 열려 있으면 `Esc`는 다이얼로그를 먼저 닫습니다.

화면 하단의 **+ Add project**에서 로컬 Git 저장소 경로를 입력해 프로젝트를 등록하고 전환합니다.
macOS에서는 **Choose folder…**로 폴더를 선택한 뒤 **Add & switch**로 등록할 수도 있습니다.
`wt -C ~/code/my-project`로 저장소를 열면 자동 등록됩니다. 프로젝트를 클릭하면 그 아래에 worktree 목록이 들여쓰기되어
펼쳐집니다. 현재 프로젝트를 다시 클릭하면 접히며, 다른 프로젝트를 선택하면 해당 목록으로 전환합니다.
worktree를 선택하면 오른쪽에 상세 정보와 실행 버튼이 표시됩니다.
`p`는 현재 프로젝트로 초점을 옮깁니다. `↑` / `↓`로 항목 이동, `→`로 펼치기·하위 항목 이동,
`←`로 상위 프로젝트 이동·접기를 할 수 있으며, `Enter`로도 프로젝트를 선택할 수 있습니다.
현재 프로젝트는 `●`, 선택한 worktree는 배경색으로 표시합니다.
프로젝트와 worktree는 각각 한 줄로 표시하며, worktree 옆에 상태와 사용 중인 포트를 표시합니다.
선택한 worktree의 전체 경로는 상세 패널에서 확인할 수 있습니다.
생성·삭제·새로고침·종료 버튼은 단축키와 함께 화면 하단에 표시됩니다.
버튼은 테두리 없는 한 줄 크기이며, 상세 동작 버튼은 정보 바로 아래 두 줄로 모았습니다.
넓은 터미널에서는 프로젝트 탐색 패널과 상세 패널을 나란히, 95열 미만에서는 세로로 배치합니다.
프로젝트 패널의 오른쪽 테두리를 마우스 왼쪽 버튼으로 드래그하면 두 패널의 너비를 조절할 수 있습니다.
별도 구분선이나 좌우 패널 사이 간격은 없습니다.
프로젝트 패널은 테두리 2열까지 줄일 수 있고, 상세 패널은 최소 44열을 유지합니다. 조절한 비율은 실행 중 유지됩니다.
공간이 부족한 프로젝트명과 워크트리 텍스트는 잘리며, 마우스를 올리면 전체 텍스트를 툴팁으로 표시합니다.
터미널이 좁아지면 너비 조절을 비활성화하고, 다시 넓어지면 이전 비율을 복원합니다.

등록 목록은 `~/.worktree-managers/.projects.json`에 저장되며 재실행 후에도 유지됩니다.
현재 폴더가 Git 저장소이면 자동 등록하고, 저장소 밖에서 `wt`를 실행하면 등록된 프로젝트를
엽니다. 등록된 프로젝트가 없으면 하단의 **+ Add project**로 시작할 수 있습니다.
같은 저장소의 다른 worktree는 중복 등록하지 않습니다. 프로젝트 전환은 실행 중인 서버를
...
엽니다(`codex app <worktree 경로>`). `codex` CLI가 PATH에 있어야 하며, 실행 오류는 우측 상단 toast에 나타납니다.
...
4. **Stop**으로 앱에서 실행한 서버와 하위 프로세스를 종료합니다.

서버 출력은 터미널에서 `wt logs <브랜치명 또는 worktree 경로>`로 확인할 수 있습니다.

행 선택, 버튼, 스크롤을 마우스로 조작할 수 있습니다. `Tab` / `Shift+Tab`으로 초점을 옮기고
`Enter`로 버튼을 실행합니다. 80열 × 24행에서도 주요 버튼을 사용할 수 있으며, 좁은 터미널에서는 세로 배치로
바뀝니다. 보이지 않는 영역은 마우스 휠이나 스크롤바로 이동하세요.
경로, 브랜치명, 패널 제목과 버튼 글자는 마우스로 드래그해 선택할 수 있습니다.
