...
상단의 **+ Add project**에 로컬 Git 저장소 경로(예: `~/code/my-project`)를 입력하면
프로젝트를 등록하고 전환합니다. **PROJECTS** 패널에서 프로젝트를 클릭하면 해당 저장소의
worktree 목록으로 전환합니다. `p`는 프로젝트 목록으로 초점을 옮기며, `Tab`과 `Enter`로도
선택할 수 있습니다. 현재 프로젝트는 `●`와 강조색으로 표시합니다.
각 항목에는 프로젝트 이름과 경로가 표시되고, 긴 경로는 줄여서 표시합니다.
선택한 프로젝트의 전체 경로는 화면 상단에서 확인할 수 있습니다.
넓은 터미널에서는 프로젝트·worktree·상세 패널을 나란히, 110열 미만에서는 세로로 배치합니다.
...
경로, 브랜치명, 로그, 패널 제목과 버튼 글자는 마우스로 드래그해 선택할 수 있습니다.
