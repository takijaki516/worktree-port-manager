...
**PROJECTS** 패널의 **+ Add project**에 로컬 Git 저장소 경로(예: `~/code/my-project`)를 입력하면
프로젝트를 등록하고 전환합니다. 프로젝트를 클릭하면 그 아래에 worktree 목록이 들여쓰기되어
...
macOS에서는 **+ Add project → Choose folder…**로 기본 폴더 선택 창을 열 수 있습니다.
선택한 경로를 확인한 뒤 **Add & switch**로 등록·전환하세요. 경로를 직접 입력하는 것도 가능합니다.

등록 목록은 `~/.worktree-managers/.projects.json`에 저장되며 재실행 후에도 유지됩니다.
현재 폴더가 Git 저장소이면 자동 등록하고, 저장소 밖에서 `wt`를 실행하면 등록된 프로젝트를
엽니다. 등록된 프로젝트가 없으면 **+ Add project**로 시작할 수 있습니다.
...
선택한 텍스트는 `Ctrl+Y`로 복사합니다. 복사는 터미널의 OSC 52 클립보드 지원이 필요합니다.
